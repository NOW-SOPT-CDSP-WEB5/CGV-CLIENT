...
function MovieInfo() {
	...
	return (
		<>
			<MovieInfoContainer>
				<MovieInfoWapper>
					<MovieInfoImg src={MovieIcons.movieInfo.icMovieTicket} alt="movieEgg" />
					<MovieInfoText>예매율 1위</MovieInfoText>
					<MovieInfoPercent>
						<Typo.Num.Number1>91.4</Typo.Num.Number1>
						<Percent src={MovieIcons.movieDetail.icMoviePercent} alt="percent" />
					</MovieInfoPercent>
				</MovieInfoWapper>
				<MovieInfoWapper>
					<MovieInfoImg src={MovieIcons.movieInfo.icMovieEgg} alt="movieTicket" />
			...
			<MovieSummaryContainer>
				<Border />
				<MovieSummaryWapper expanded={summaryBtn}>
				...
				<MovieSummaryButton type="button" onClick={handleSummaryBtn}>
					<SummaryButtonText>{summaryBtn ? '접기' : '자세히 보기'}</SummaryButtonText>
				</MovieSummaryButton>
			</MovieSummaryContainer>
		</>
	);
...
interface MovieSummaryWapperProps {
	expanded: boolean;
}

const MovieSummaryWapper = styled(Typo.Body.Body3M14)<MovieSummaryWapperProps>`
	display: -webkit-box;
	width: 33.5rem;
...
const SummaryButtonText = styled(Typo.Title.Title2SB15)`
